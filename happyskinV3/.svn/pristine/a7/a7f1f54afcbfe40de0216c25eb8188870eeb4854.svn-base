...
export default function cosmeticsReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case CM_PRODUCT_UPDATE_SUCCESS:
      ...
      return state.setIn(['fetchingUpdate'], false).removeIn(['producted']).setIn(['producted'], producted).removeIn(['producting']).setIn(['producting'],newPro).setIn(['showRelate'], true).setIn(['productRelate'], action.productRelate);
    ...
    case CM_DETAIL_REQUEST:
      return state.setIn(['isFetching'], true).setIn(['showRelate'], null);
