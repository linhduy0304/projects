    ...
    return (
        <TouchableOpacity onPress={() => this.detailTrending(item)} style={styles.slide}>
          ...
          <View style={styles.BoxAction}>
            <BoxAction view={item.data.count_views} like={item.data.count_like} comment={item.data.count_comment} isLike={item.data.is_like_by_current_id} itemId={item.data.id} itemType={item.data.type} itemFeedType={(item.data.feedtype) ? item.data.feedtype : null} itemTitle={item.data.title} />
          </View>
        </TouchableOpacity>
    );
  }
