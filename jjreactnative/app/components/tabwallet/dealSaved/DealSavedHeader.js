                    ...
                    <View>
                        <View style={{
                            ...
                            <Text style={{fontWeight: 'bold', color: '#454545', fontSize: 18}}>
                                {strings('label.collection').toUpperCase()}
                            </Text>
    ...
    _renderCollectionSavedItem = ({item, index}) => {
        ...
        return (
            <View style={{paddingLeft: DIMENSION_PADDING_MEDIUM, flexDirection: 'row'}}>
                <CollectionSavedItem collection={item.collection}
                                     navigation={this.props.navigation}
                                     onUnSaveClicked={this.props.onUnSaveClicked}/>
                {
                    latestItem &&
                    <View style={{width: DIMENSION_PADDING_MEDIUM}}/>
                }
            </View>
        )
    }

    _getKeyExtractor = (item, index) => !!item.collection ? item.collection.id : 'collection' + '_' + index;
