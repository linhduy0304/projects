...
class DanhDau extends Repository {
    ...
    render() {
        console.log('DanhDau:render', this.state);
        return (
            <Container>
                {/* Toolbar */}
                <JJHeader
                    navigation={this.props.navigation}
                    title={"KHUYẾN MÃI ĐÁNH DẤU"}
                />
                {/* Content */}
    ...
    _renderDeal = ({item}) => {
        if (item.type !== undefined && item.type !== null && item.type === -1) {
            return (
                <Text style={styles.header}>{strings('wallet.deal_saved.header')}</Text>
            )
        }
    ...
    _renderHeader = () => <DealSavedHeader navigation={this.props.navigation}
    ...
    _onLoadMore = () => this.state.hasMore && this._fetchDealSaved()
