...
import RateDealItem from './RateDealItem';
import {COLOR_GRAY_BG, COLOR_TEXT_INACTIVE} from '../../../../../resources/colors';
import { connect } from 'react-redux';
import {
    DIMENSION_PADDING_MEDIUM, DIMENSION_TEXT_CONTENT,
...
class RateDealDetail extends BaseComponent {
    ...
    _keyGenerator = (item, index) => `rate_deal_${item.id}_${index}`;
    ...
    _renderItem = ({item, index}) => <RateDealItem key={'rate_item_' + item.id} rate={item} navigation={this.props.navigation} />;
    ...
    _fetchData = (refresh) => {
        ...
        dealApi.getJAMJARateByDeal(this.props.deal.id, refresh ? 0 : this.state.rateList.length, ITEM_LIMIT)
            .then(result => {
                console.log(result)
...
export default connect(mapStateToProps)(RateDealDetail);
