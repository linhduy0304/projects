...
import { View, StyleSheet, FlatList, TouchableOpacity, Easing } from 'react-native';
...
class AllStore extends Component<Props> {
        ...
        return (
            <View>
                ...
                <View style={styles.rateWrapper}>
                    ...
                    <View>
                        ...
                        <Text style={{ color: COLOR_TEXT_INACTIVE, fontSize: DIMENSION_TEXT_SUB }}>
                            Cho tất cả {this.state.stores.length} cửa hàng
                        </Text>
                    </View>
                </View>
                ...
                <Text style={styles.description}>
                    Điểm đánh giá qua Foody trên đây là điểm đánh giá trung bình của tất cả
                    {' ' + this.state.stores.length + ' '}
                    cửa hàng thuộc thương hiệu
                    {' ' + deal.brand.brand_name + ' '}
                    tại
    ...
    calculateDistance(item) {
        if (!!this.props.latitude && !!item.latitude) {
    ...
    fetchData = (refresh) => {
        ...
        dealApi.getFoodyReviewByBrand(this.props.deal.brand.id, refresh ? 0 : this.state.stores.length, ITEM_LIMIT)
            .then(result => {
                console.log(result)
...
export default connect(mapStateToProps)(AllStore);
