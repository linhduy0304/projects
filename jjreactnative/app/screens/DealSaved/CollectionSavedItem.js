...
import { DIMENSION_BUTTON_MEDIUM, DIMENSION_PADDING_MEDIUM, DIMENSION_RADIUS_MEDIUM, DIMENSION_TEXT_CONTENT } from '../../resources/dimens';
...
export default class CollectionSavedItem extends BaseComponent {
    ...
    render() {
        ...
        return (
            <TouchableOpacity activeOpacity={1} style={{ height: 135, width: 180 }} onPress={this._onPressed}>
                <FastImage
                    style={styles.image}
                ...
                <View style={{ flexDirection: 'row', position: 'absolute', bottom: 0, padding: DIMENSION_PADDING_MEDIUM }}>
                    <View style={{ flexDirection: 'column', width: 100 }}>
                        <Text style={{ fontSize: DIMENSION_TEXT_CONTENT, color: '#EDEDED' }}>{collection.deal_count} khuyến mãi</Text>
                        <Text style={{ fontSize: DIMENSION_TEXT_CONTENT, color: '#ffffff', fontWeight: 'bold' }} numberOfLines={3}>
                            {collection.title}
                        </Text>
                    </View>
                    <TouchableOpacity
                        style={{ flex: 1, flexDirection: 'column', justifyContent: 'space-between', width: DIMENSION_BUTTON_MEDIUM }}
    ...
    _onPressed = () => {
        ...
        AnalyticsUtil.logNormalEvent('saved_list_open_collection_detail', baseLogParams, 'collection');
