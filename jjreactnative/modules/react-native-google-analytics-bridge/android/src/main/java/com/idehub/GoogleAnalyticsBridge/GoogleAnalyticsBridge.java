...
public class GoogleAnalyticsBridge extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void trackProductAction(String trackerId, ReadableMap product, String actionName, String actionValue){
        Tracker tracker = getTracker(trackerId);

        if (tracker != null) {

            Product pro = this.getPurchaseProduct(product);
            ProductAction productAction = new ProductAction(actionName);
            productAction.setProductListSource(actionValue);

            HitBuilders.ScreenViewBuilder builder = new HitBuilders.ScreenViewBuilder()
                    .addProduct(pro)
                    .setProductAction(productAction);

            tracker.setScreenName("deal_detail_view");
