...
RCT_EXPORT_METHOD(trackProductAction:(NSString *)trackerId product:(NSDictionary *)product actionName:(NSString *)actionName actionValue:(NSString *)actionValue)
{
    id<GAITracker> tracker = [[GAI sharedInstance] trackerWithTrackingId:trackerId];
    NSString *productId = [RCTConvert NSString:product[@"id"]];
    NSString *productName = [RCTConvert NSString:product[@"name"]];
    ...
    [action setProductListSource:actionValue];
    [builder setProductAction:action];
    // Sets the product for the next available slot, starting with 1.
    [builder addProduct:ecommerceProduct];
    [tracker set:kGAIScreenName value:@"deal_detail_view"];
    [tracker send:[builder build]];
}
