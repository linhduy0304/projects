...
class CartAddress extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      name: '',
  ...
  add() {
    Keyboard.dismiss();
    const {name, mobile, address, city, district, ward, district_id} = this.state;
    ...
    this.props.addAddress(body, this.props.action)
  }
  ...
  render() {
    ...
    return (
      <View style={{flex: 1, backgroundColor: '#e6eff1'}}>
        ...
        <Nav label='THÊM ĐỊA CHỈ GIAO HÀNG'>
          <TouchableOpacity onPress={() => this.add()} style={{padding: 15, position: 'absolute', right: 0}}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    addAddress: (body, action) => dispatch(addAddress(body, action)),
  }
}
