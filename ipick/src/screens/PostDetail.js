...
class PostDetail extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.post.postDetail && nextProps.post.postDetail !== this.props.post.postDetail) {
  ...
  _onNavigationStateChange(webViewState){
      if(webViewState.url != ('https://mapi.ipick.vn/api/v1/webview?slug=' +this.props.post.postDetail.slug)) {
        this.refs.web.stopLoading();
        Linking.openURL(webViewState.url);
      }
      this.setState({
        height: webViewState.title
      });
  ...
  loadDetail(slug) {
    ...
    this.props.actions.postDetailRequest();
    this.props.actions.postDetail(slug)
  }
      ...
      <View style={css.container}>
            ...
            <ScrollView
              ...
              <View style={[styles.ctLink,{marginTop: 0}]}>
                ...
                <View style={{paddingTop: 15, paddingRight: 15}}>
                {
                  ...
                  <Web
                    ref='web'
                    source={{uri: 'https://mapi.ipick.vn/api/v1/webview?slug=' +this.props.post.postDetail.slug}}
                    scrollEnabled={false}
                    // javaScriptEnabled
              ...
              {
                this.props.post.relate.length != 0 ?
                  <View style={[styles.ctLink,{marginTop: 0}]}>
                    <View style={styles.ctHeader1}>
                      <View style={[styles.ctTxtLink, {borderBottomColor: 'rgb(0, 193, 125)'}]}>
                        <Text style={styles.txtLink}>Liên quan</Text>
                      </View>
                    </View>
                    <View style={{marginTop: 15, marginRight: 15}}>
                      {
                        this.props.post.relate.map((item, index) => {
                          return (
                            <View key={index} style={styles.ctItem}>
                              <TouchableOpacity onPress={() => this.loadDetail(item.slug)}>
                                <Image style={{height: 80*400/600, width: 80}} source={{uri: item.image_thumb+'_600x400.png'}} />
                              </TouchableOpacity>
                              <View style={{marginLeft: 10, flex: 1}}>
                                <Text numberOfLines={2} onPress={() => this.loadDetail(item.slug)} style={styles.txtTitleRelate}>{item.title}</Text>
                                <Text style={styles.txtBy}>Đăng bởi: <Text onPress={() => this.profile(item.user.id)} style={{color: 'rgb(0, 139, 125)'}}>{item.user.full_name}</Text></Text>
                              </View>
                            </View>
                          )
                        })
        ...
        <Animated.View style={[styles.navScroll,{opacity: navOpacity, transform: [{translateY: navTranslate}]}]}>
          <NavBar style={{navBar: css.navBar, statusBar: css.statusBar}} statusBar={{barStyle: 'dark-content', backgroundColor: '#fff'}} >
            ...
            <NavButton onPress={() => this.props.post.postDetail ? this.openShare() : null} style={css.navBack}>
