...
class EditVideo extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      errorLink: null,
      link: '',
      url_videos: this.props.data.url_videos,
      video: '',
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    //   var url = this.state.url_videos;
    //   url.push(nextProps.post.link.full_url)
    //   this.setState({
  ...
  send() {
    ...
    if(this.state.url_videos.length == 0) {
      this.setState({
        error: 'Bạn phải thêm đường dẫn Youtube'
      })
      this.refs.scroll.scrollTo({x: 0, y: 0, animated: true})
      return
    }
    ...
    var data = {
      ...
      atribute_url_video: this.state.url_videos
    }
