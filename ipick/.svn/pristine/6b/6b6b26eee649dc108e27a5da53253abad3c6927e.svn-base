...
class Follower extends Component {
  ...
  componentWillMount() {
    this.props.actions.followerRequest();
    var firebase = RNFirebase.initializeApp({ debug: false });
    firebase.analytics().logEvent("Follower_Screen");
  }

  componentDidMount() {
    this.props.actions.follower(null, this.props.id);
    // timeout = setTimeout( () => {
    // }, 1000);
  ...
  _onRefresh() {
    ...
    this.props.actions.follower(null, this.props.id, 'RF');
    this.setState({
      isRefreshing: false,
    })
  }
  ...
  loadMore() {
    ...
    this.props.actions.follower(null, this.props.id, 'LM', this.state.page + 1);
    this.setState({
  ...
  renderFollow() {
    if(this.props.id == this.props.profile.currentUser.id) {
      return 'Bạn có '+this.props.profile.currentUser.count_followers+' người follow.';
    }else 
      if(this.props.profile.otherProfile) {
        return this.props.profile.otherProfile.full_name+' có '+this.props.profile.otherProfile.count_followers+' người follow.';
      }else return null
  }
        ...
        <NavBar style={{navBar: css.navBar, statusBar: css.statusBar}} statusBar={{barStyle: 'dark-content', backgroundColor: '#fff'}} >
          <NavGroup>
            ...
            <View style={{marginLeft: 9,justifyContent: 'center'}}>
              <Text style={css.txtTitle}>Follower</Text>
            </View>
          </NavGroup>
        </NavBar>
        ...
        <View style={{padding: 15}}>
          ...
          <View style={styles.ctSearch}>
            ...
            <TextInput
              ...
              placeholder="Tìm kiếm follower"
              underlineColorAndroid="transparent"
              placeholderTextColor="rgb(194, 197, 208)"
              onChangeText={(keyword) => this.setState({keyword: keyword})}
              onSubmitEditing={() => this.props.actions.follower(this.state.keyword, this.props.id)}
        ...
        </View>
        {
          ...
          this.props.profile.follower ?
            ...
            <FlatList
              ...
              data={this.props.profile.follower}
              refreshControl={
                <RefreshControl
