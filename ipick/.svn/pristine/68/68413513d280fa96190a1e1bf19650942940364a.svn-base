...
export function createNewPass(body) {
  return dispatch => {
...
export function myPickSuccess(data, total, action, id) {
  return {
    ...
    Profile.myPick(id, page).then((res) => {
      ...
      switch(res.status) {
        case 500:
          ...
          return;
        case 200:
            dispatch(myPickSuccess(res.data,res.total, action, id))
          return;
        default: 
          dispatch(myPickFail());
          return;
...
export function myPostSuccess(data, total, action, id) {
  return {
    ...
    Profile.myPost(id, page).then((res) => {
      switch(res.status) {
        case 500:
          ...
          return;
        case 200:
            dispatch(myPostSuccess(res.data, res.total, action, id))
          return;
        default: 
          dispatch(myPostFail());
          return;
