...
class PostAudio extends Component {
  ...
  send() {
    ...
    this.props.actions.postLink(data, 'audio');
      ...
      <View style={{padding: 15}}>
       {
        ...
        <View style={{flexDirection: 'row', marginTop: 15}} >
          ...
          {
            this.state.image ?
              <Image style={{height:50, width: 50, marginBottom: 10 }} source={{uri: this.state.image}} />
            : null
          }
          </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(PostAudio);
