...
class MyPost extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.tab.profile && nextProps.profile.myPost && nextProps.profile.myPost.length !== 0) {
      setTimeout( () => {
  ...
  loadMore() {
    if (!this.props.profile.lmMyPost) {
        return;
    }
      ...
      <View style={css.container}>
        {
          ...
          :
          this.props.profile.myPost ?
            this.props.profile.myPost.length == 0 ?
            ...
            <FlatList
              ...
              data={this.props.profile.myPost}
              refreshControl={
                <RefreshControl
