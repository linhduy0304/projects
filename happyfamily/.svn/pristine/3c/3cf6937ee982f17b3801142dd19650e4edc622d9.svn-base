...
const {
  ...
  REQUEST_RELATIONSHIP_REQUEST,
  REQUEST_RELATIONSHIP_SUCCESS,
  REQUEST_RELATIONSHIP_FAIL,
...
export function requestRelationshipRequest() {
  return {
    type: REQUEST_RELATIONSHIP_REQUEST,
  }
}

export function requestRelationshipSuccess(data) {
  return {
    type: REQUEST_RELATIONSHIP_SUCCESS,
    data: data
  }
}

export function requestRelationshipFail() {
  return {
    type: REQUEST_RELATIONSHIP_FAIL,
  }
}

export function requestRelationship(body) {
  return dispatch => {
    dispatch(requestRelationshipRequest());
    return Individual.requestRelationship(body).then(res => {
      switch (res.status) {
        case 200:
          dispatch(requestRelationshipSuccess());
        ...
        case 500:
          ...
          dispatch(requestRelationshipFail())
          return;
        default:
          SimpleToast.show(res.errors)
          dispatch(requestRelationshipFail());
