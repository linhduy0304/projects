...
class AddIndividual extends React.Component {
    ...
    render() {
        ...
        var { value, hideResults, show } = this.state;
        var { itemEditing } = this.props.individual;
        return (
            <View style={styles.container}>
                ...
                <KeyboardAvoidingView behavior='padding' keyboardVerticalOffset={Platform.OS === 'android' ? -500 : 0} style={{ flex: 1 }} >
                    <ScrollView style={styles.content}
                        ...
                        <View style={styles.boxInfo}>
                            ...
                            {
                                dataSearch.length > 0 && show ?
                                <View >
                                    {
                                        dataSearch.map((item, index) => {
                                            return (
                                                <TouchableOpacity key={index} onPress={() => this.onSelectUser(item)} style={styles.btnSelect}>
                                                    <Image style={styles.avatar} source={{uri: item.avatar+'_100x100.png'}} />
                                                    <Text style={styles.itemText}>
                ...
                <Modal 
                    ...
                    >
                        <PageTypeEvent date = {this.state.date} lunar= {this.state.lunar} lunarValue= {this.state.lunarValue} close={() => this.setState({modalTypeEvent: false})} individual={itemEditing} />
                </Modal>
            </View>
        )
    }
