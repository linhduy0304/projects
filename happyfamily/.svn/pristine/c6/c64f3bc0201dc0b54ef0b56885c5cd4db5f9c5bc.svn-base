...
export function logout() {
  return dispatch => {
    ...
    new Store().deleteSessionToken(Constant.HF_DATA_USER);
    new Store().deleteSessionToken(Constant.HF_HEADER_REQUEST);
    new Store().deleteSessionToken(Constant.HF_IS_LOGIN);
    new Store().deleteSessionToken(Constant.HF_IS_PASS);
    // emptyDataUser();
    ...
    .then(function (res) {
      switch(res.status) {
        case 200:
          ...
          new Store().storeSession(Constant.HF_DATA_USER, res.data);
          new Store().storeSession(Constant.HF_HEADER_REQUEST, headers);
          new Store().storeSession(Constant.HF_IS_LOGIN, true);
          // new Store().storeSession(Constant.HF_IS_PASS, true);
...
export function dataUser() {
  return dispatch => {
    new Store().getSession(Constant.HF_HEADER_REQUEST).then(headers => {
...
export function checkLogin() {
  return dispatch => {
    new Store().getSession(Constant.HF_IS_LOGIN).then((is_login) => {
      if(is_login) {
        new Store().getSession(Constant.HF_IS_PASS).then(is_pass => {
          if(is_pass) {
            new Store().getSession(Constant.HF_HEADER_REQUEST).then(headers => {
              return Auth.checkLogin(headers).then(res => {
                switch(res.status) {
                  case 200:
                    ...
                    if(res.data.is_first_login == 1) {
                      new Store().storeSession(Constant.HF_IS_FIRST_LOGIN, true);
                    }
                    dispatch(profileUserSuccess(res.data));
                    Actions.tab({type: 'reset'});
                    return;
        ...
        new Store().getSession(Constant.HF_IS_FIRST).then((is_first) => {
          if(is_first) {
            Actions.login({type: 'reset'})
          }else {
