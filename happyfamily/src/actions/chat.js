...
//chat list
export function chatRequest(action) {
  return {
    type: CHATS_REQUEST,
    action
  }
...
export function chatsSuccess(data, action) {
  return {
    type: CHATS_SUCCESS,
    data: data, 
    action
...
export function chats(action = 'L', page = 1) {
  return dispatch => {
    dispatch(chatRequest(action));
    return Chat.chats(page).then(res => {
      switch(res.status) {
        case 200:
          dispatch(chatsSuccess(res.data.conversations, action))
          return;
        case 500: 
          Alert.alert(
            'Thông báo!',
