...
class User extends PureComponent {
  ...
  componentWillMount() {
    this.props.loadSearch('user', this.props.keyword, 'L');
  }
  ...
  _onRefresh() {
    ...
    this.props.loadSearch('user', this.props.keyword, 'RF');
    this.setState({
      isRefreshing: false,
    })
  }
  ...
  loadMore() {
    ...
    this.props.loadSearch('user', this.props.keyword, 'LM', this.state.page + 1);
    this.setState({
      ...
      <View style={[css.container, {backgroundColor: 'rgb(237, 239, 241)'}]}>
        {
          ...
          :
          this.props.search.searchUser ?
            this.props.search.searchUser.length == 0 ?
              <NoData />
            : 
            <FlatList
              ref="listRef"
              ...
              data={this.props.search.searchUser}
              refreshControl={
                <RefreshControl
              ...
              renderItem={(data) => (
                <View style={styles.ctItem}>
                  <TouchableOpacity >
                    <Image style={{height: 80, width: 80, borderRadius: 40}} source={data.item.avatar ? {uri: data.item.avatar+'.png'} : require('../../images/avatar_default.png')} />
                  </TouchableOpacity>
                  <View style={{marginLeft: 10, flex: 1}}>
                    <Text numberOfLines={2}  style={styles.txtTitle}>{data.item.full_name}</Text>
