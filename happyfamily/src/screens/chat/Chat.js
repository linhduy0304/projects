...
class Chat extends React.Component {
  ...
  detailChat(data) {
    if(data.type === 'multiple') {
      Actions.chatGroup({title: data.name, id: data.id, socket: this.props.socket})
      // this.props.navigation.navigate('chat_group', {title: data.full_name})
    }else {
      Actions.chatOne({title: data.user.full_name, id: data.id, socket: this.props.socket})
...
const mapDispatchToProps = (dispatch) => {
  return {
    chats: (action, page) => dispatch(chats(action, page)),
  }
}
